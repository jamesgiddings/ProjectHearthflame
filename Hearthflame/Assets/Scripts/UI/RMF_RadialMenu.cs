...
public class RMF_RadialMenu : MonoBehaviour {
    ...
    [SerializeField] private float anglePortion;
    public float AnglePortion => anglePortion;
    ...
    void Awake() {
        ...
        angleOffset = (anglePortion / (float)elementCount);
    ...
    void Update() {
        if (isActiveMenu)
		{
            ...
            if (angleOffset != 0 && useLazySelection)
            {
                ...
                index = (int)Mathf.Clamp((currentAngle / angleOffset), 0f, (float)elementCount - 1);
    ...
    private float normalizeAngle(float angle) {
        ...
        if (!useGamepad)
		{
            if (angle < 0)
                angle = 0;
            currentAngle = Mathf.Clamp(angle, 0, anglePortion);
        }
